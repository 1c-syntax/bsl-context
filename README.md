...
- **Блок кода и футер страницы.** Пример кода в HBK — таблица с фоном
  `#f7f7f7`; распознаётся по ней, а не по чаптеру «Пример:», потому что
  на части страниц слово «Пример:» вписано прямо в текст описания через
  `<BR>`, а таблица лежит внутри незакрытого `<p>`
  (`ВнешнийОтчет.ОбработкаПроверкиЗаполнения`) — иначе код целиком утекал
  бы в описание. Внизу каждой из 25503 страниц стоит `<HR>` и ссылка
  «Методическая информация» на сайт 1С: разбор обрывается на этой линии,
  иначе футер приклеивается к последней открытой секции — например
  к синтаксису конструктора (`Новый ГрафическаяСхема`).
...
- **Имя контекста — с заголовка страницы** (`V8SH_pagetitle`), а не из
  ...
  и число неуникальных имён падает с 27 до 10.
  Исключение — страница, на которую ведёт несколько узлов оглавления: так
  платформа оформляет переименования (`PlannerCommandSource.html` — это и
  «ИсточникКомандПланировщика», устаревший с 8.3.23, и
  «ИсточникКомандПоляПланировщика», причём наборы значений у них разные).
  Для таких узлов имя берётся из оглавления — только оно их и различает.
